...
def predict_image_class(model_to_use, path):
    ...
    #Make predictions
    pred = model_to_use.predict(input_arr)
    predictions_around = np.around(pred)
    
    if predictions_around == [[0]]:
        class_pred = "No smoke"
        print("No smoke . Class : ", predictions_around[0])
    else:
        class_pred = "Smoke"
        print("Smoke in image / Class : ", predictions_around[0])
